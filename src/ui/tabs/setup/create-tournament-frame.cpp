	...
	: QFrame() // QGroupBox(obs_module_text("setup.createTournament.title"))
{
	...
	this->_tournamentTitleLineEdit->setPlaceholderText(obs_module_text(
		"setup.createTournament.titleInputPlaceholder"));
	...
	this->_tournamentTypeComboBox->addItem(
		obs_module_text("tournamentType.custom"),
	...
	auto endTournamentCreationButton = new QPushButton(obs_module_text(
		"setup.createTournament.endTournamentCreationButton"));
	this->connect(endTournamentCreationButton, &QPushButton::clicked,
		      [this]() { this->endTournamentCreation(); });
...
bool CreateTournamentFrame::endTournamentCreation()
{
	...
	auto tournamentType =
		(Tournament::Type)this->_tournamentTypeComboBox->currentData()
			.toInt();
	...
	for (int playerIndex = 0;
	     playerIndex < this->_playerListLayout->count(); playerIndex++) {
		...
		if (playerName == "") {
			snprintf(errorMessage, errorMessageBufferSize,
				 "Player %d: %s", playerIndex + 1,
				 obs_module_text(
					 "error.setup.missingPlayerName"));
		...
		if (playerImagePath == "") {
			snprintf(errorMessage, errorMessageBufferSize,
				 "Player %d: %s", playerIndex + 1,
				 obs_module_text(
					 "error.setup.missingPlayerImage"));
			auto errorDialog = new ErrorDialog(this, errorMessage);
			errorDialog->show();
			return false;
		}
		...
		if (playerDescription == "") {
			snprintf(
				errorMessage, errorMessageBufferSize,
				"Player %d: %s", playerIndex + 1,
				obs_module_text(
					"error.setup.missingPlayerDescription"));
			auto errorDialog = new ErrorDialog(this, errorMessage);
			errorDialog->show();
			return false;
		}
	...
	switch (tournamentType) {
		...
		// 		newTournamentTitle);
		// 	pluginData->tournaments.push_back(
		// 		newTournament); // NOTE: needs to be before CustomKnockoutTabFrame initialization

...
CreatePlayerFrame::CreatePlayerFrame() : QFrame()
{
	...
	this->_playerNameLineEdit->setPlaceholderText(obs_module_text(
		"setup.createTournament.playerNameInputPlaceholder"));
	...
	auto playerImageOpenFileDialogButton = new QPushButton(obs_module_text(
		"setup.createTournament.playerChooseImagePathButton"));
	this->connect(
		...
		[this, emptyPlayerImageLabelText]() {
			...
			playerImageFileDialog->setFileMode(
				QFileDialog::ExistingFile);
			playerImageFileDialog->setNameFilter(
				"*.PNG *.png *.JPG *.jpg *.jpeg");
			playerImageFileDialog->exec();

			auto selected = playerImageFileDialog->selectedFiles();
			if (selected.size() == 0) {
				this->_playerImageLabel->setText(
					emptyPlayerImageLabelText);
				this->_imagePath = "";
	...
	playerImageInputLayout->addWidget(
		new QLabel(obs_module_text(
			"setup.createTournament.playerImageInputLabel")),
		0);
	...
	auto playerDescriptionInput = new AppLabeledInput(
		obs_module_text(
			"setup.createTournament.playerDescriptionInputLabel"),
